...
IUSE="berkdb doc gdbm perl_flavor_debug perl_flavor_ithreads"
...
src_configure()	{ eblit-run src_configure v50160002 ; }
...
for x in setup ; do
	e="${FILESDIR}/eblits/pkg_${x}-v50160002.eblit"
	if [[ -e ${e} ]] ; then
		. "${e}"
		eval "pkg_${x}() { eblit-run pkg_${x} v50160002 ; }"
	fi
done
